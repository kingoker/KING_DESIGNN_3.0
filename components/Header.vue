<template>
  ...
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative h-16 flex items-center">
      <!-- Логотип -->
      ...
      <NuxtLink
        to="/login"
        aria-label="Login"
        class="z-10 text-2xl text-[#00ffd1] hover:text-[#00fff1] drop-shadow-[0_0_6px_rgba(0,255,209,0.8)] transition-colors duration-200 absolute left-1/2 transform -translate-x-1/2 md:static md:ml-auto"
      >
        👑
      </NuxtLink>

      <!-- Десктопное меню -->
      <nav class="hidden md:flex space-x-8 z-10 ml-auto">
        <NuxtLink
          v-for="link in links"
          :key="link.text"
          :to="link.to"
      ...
      <transition name="fade">
        <div
          v-if="mobileMenuOpen"
          class="md:hidden absolute top-full left-0 w-full bg-black/30 backdrop-blur-lg border-t border-[#00ffd1]/50"
        >
